...
Gangnam = pd.DataFrame()
yymm = []
yymm = pd.date_range("2011-01", "2031-01", freq="M")
print(len(yymm))
Gangnam['yymm'] = yymm[:110]
Gangnam['price'] = data['강남구']
# Gangnam.set_index('yymm', inplace=True)
print(Gangnam.head(3), Gangnam.tail(3), Gangnam.shape)
'''                   price                           price
    yymm                           yymm
...
'''
print(Gangnam.info())
timeSeries = Gangnam.loc[:, ['yymm', 'price']]
...
# plt.plot(ts)
# plt.title("Gangnam APT price 2011-01 ~ 2021-12")
...
# predict
start_index = yymm[110] # 2021-01-31 00:00:00
end_index = yymm[239]   # 2021-12-31 00:00:00
forecast = model_fit.predict(start=start_index, end=end_index, typ='levels')
# # 실제값이랑 비교하기
# print(Gangnam.tail(12))
# print(forecast)
'''         - 실제값 -           - forecast -
          yymm        price          price
...
# # 시각화
plt.figure(figsize=(15, 8))
plt.plot(Gangnam.yymm, Gangnam.price, label="original")
plt.plot(forecast, label='predicted')
plt.title("Gangnam-gu APT price Forecast")
plt.xlabel("Year-Month")
...
# # 잔차 분석 : 잘 안 됨...
# 어떠한 패턴이나 특성이 없는지 확인!
# resi = np.array(Gangnam[Gangnam.yymm>=start_index].price) - np.array(forecast)
# plt.figure(figsize=(15, 8))
# plt.plot(Gangnam[Gangnam.yymm>=start_index], resi)
# plt.xlabel("Year-Month")
# plt.ylabel("Residual")
# plt.legend()
# plt.show()
...
#     return df
#
# # score_check(np.array(Gangnam[Gangnam.yymm>=start_index].price), np.array(forecast))
# print(score_check(np.array(Gangnam[Gangnam.yymm>=start_index].price), np.array(forecast)))
# ==>        R2   Corr     RMSE    MAPE
